        ...
        : <div className="overflow-x-auto"><table className="w-full">
            ...
            <tbody>{list?.map((r: ReportCard) => (
              <tr key={r.id} className="border-t border-border hover:bg-gray-50/50">
                ...
                <td className="p-3 text-sm">{r.term_id}</td>
                <td className="p-3 text-sm">{r.stream_id}</td>
          ...
          <div className="bg-white rounded-xl shadow-xl w-full max-w-lg mx-4" onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <div className="grid grid-cols-3 gap-4">
                ...
                <div><label className="block text-sm font-medium text-gray-700 mb-1">Stream ID *</label><input type="number" value={form.stream_id || ''} onChange={setF('stream_id')} required className="w-full px-3 py-2 border border-border rounded-lg text-sm outline-none focus:ring-2 focus:ring-primary" /></div>
              </div>

...
export default function LoginPage() {
  ...
  const error = loginMutation.error?.response?.data?.message || loginMutation.error?.message;
    ...
    <AuthLayout title="Sign In" subtitle="Welcome back to SSMS" heroImage="/images/school_classroom.jpg">
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="text-right -mt-3">
          <Link to={ROUTES.AUTH.FORGOT_PASSWORD} className="text-xs text-primary hover:underline font-medium">Forgot password?</Link>
        </div>
        {error && (
        ...
        <button type="submit" disabled={loginMutation.isPending} className="w-full inline-flex items-center justify-center px-8 py-3.5 rounded-lg bg-primary text-white hover:bg-primary-dark transition-colors font-medium text-sm disabled:opacity-50 cursor-pointer">
        ...
        <p className="text-center text-sm text-muted pt-2">
          ...
          <Link to={ROUTES.AUTH.REGISTER} className="text-primary hover:underline font-medium">Register your school</Link>
        </p>
      </form>
    </AuthLayout>
  );
